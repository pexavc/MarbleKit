...
//        fileprivate(set) var copy : CopyKernel!
        fileprivate(set) var downsample : DownsampleKernel!
        fileprivate(set) var paddedDownsample : PaddedDownsampleKernel!
        fileprivate(set) var transform : TransformKernel!
        fileprivate(set) var watermark : WatermarkKernel!
...
//            bgr2rgb = BGR2RGBKernel(context: context)
    ...
    //Single-used kernels
    fileprivate(set) public var kernels : Kernels = Kernels()

//
//  SWScale.swift
//  MarbleKit.Player-iOS
//
// https://github.com/kingslay/KSPlayer/blob/develop/Sources/KSPlayer/MEPlayer/Resample.swift
...
protocol Swresample {
    func transfer(avframe: UnsafeMutablePointer<AVFrame>) throws -> MarblePlayerFrame
    func shutdown()
}
...
class VideoSwresample: Swresample {
    ...
    func transfer(avframe: UnsafeMutablePointer<AVFrame>) throws -> MarblePlayerFrame {
        ...
        if let pixelBuffer = frame.corePixelBuffer {
            pixelBuffer.colorspace = MarblePlayerOptions.colorSpace(ycbcrMatrix: pixelBuffer.yCbCrMatrix, transferFunction: pixelBuffer.transferFunction)
        }
...
class AudioSwresample: Swresample {
    ...
    init(audioDescriptor: AudioDescriptor, audioFormat: AVAudioFormat) {
        ...
        MarblePlayerLog("out channelLayout: \(outChannel)")
...
public class AudioDescriptor: Equatable {
    ...
    func audioFormat(channels: AVAudioChannelCount) -> AVAudioFormat {
        ...
        commonFormat = .pcmFormatFloat32
        interleaved = MarblePlayerOptions.isUseAudioRenderer

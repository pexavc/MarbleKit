...
//  MarbleKit
//
//  Based on: https://github.com/kingslay/KSPlayer/blob/develop/Sources/KSPlayer/Metal/MetalRender.swift
...
class MetalRender {
    ...
    func clear(drawable: CAMetalDrawable) {
    ...
    func draw(pixelBuffer: CVPixelBuffer, display: DisplayEnum = .plane, texture: MTLTexture) {
        let inputTextures = pixelBuffer.textures()
    ...
    static func makePipelineState(fragmentFunction: String, isSphere: Bool = false, bitDepth: Int32 = 8) -> MTLRenderPipelineState {
        ...
        descriptor.colorAttachments[0].pixelFormat = MarblePlayerOptions.colorPixelFormat(bitDepth: bitDepth)

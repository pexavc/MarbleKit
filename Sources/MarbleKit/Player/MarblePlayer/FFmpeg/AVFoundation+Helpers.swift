...
//
//  Based on: https://github.com/kingslay/KSPlayer/blob/develop/Sources/KSPlayer/MEPlayer/AVFoundationExtension.swift
...
extension AudioUnit {
    var channelLayout: UnsafeMutablePointer<AudioChannelLayout> {
        ...
        MarblePlayerLog("audio unit channelLayout tag: \(tag)")
        if tag == kAudioChannelLayoutTag_UseChannelDescriptions {
            MarblePlayerLog("audio unit channelLayout channelDescriptions: \(layout.channelDescriptions)")
            return layout
        }
...
extension AVAudioChannelLayout {
    func channelLayout() -> AVChannelLayout {
        MarblePlayerLog("MarblePlayerOptions channelLayout: \(layout.pointee)")
        var mask: UInt64?
        if layoutTag == kAudioChannelLayoutTag_UseChannelDescriptions {
            ...
            layout.channelDescriptions.forEach { description in
                ...
                MarblePlayerLog("MarblePlayerOptions channelLayout label: \(label)")
                let channel = label.avChannel.rawValue
                MarblePlayerLog("MarblePlayerOptions channelLayout avChannel: \(channel)")
                if channel >= 0 {
                    newMask |= 1 << channel
                }
            }
        ...
        if let mask {
            // Cannot use AV_CHANNEL_ORDER_CUSTOM
            av_channel_layout_from_mask(&outChannel, mask)
        } else {
            av_channel_layout_default(&outChannel, Int32(channelCount))
        }
        MarblePlayerLog("out channelLayout mask: \(outChannel.u.mask) nb_channels: \(outChannel.nb_channels)")
        return outChannel
    }
}
...
let layoutMapTuple =
    [(tag: kAudioChannelLayoutTag_Mono, mask: swift_AV_CH_LAYOUT_MONO),
     ...
     //     (tag: kAudioChannelLayoutTag_Logic_Atmos_5_1_2, mask: swift_AV_CH_LAYOUT_7POINT1_WIDE_BACK), // Logic+Dolby Atmos?
    ]

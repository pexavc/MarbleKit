...
extension AVCodecParameters {
    mutating func ceateContext(options: MarblePlayerOptions) throws -> UnsafeMutablePointer<AVCodecContext> {
...
extension AVChannelLayout: CustomStringConvertible {
    ...
    var layoutTag: AudioChannelLayoutTag {
        MarblePlayerLog("FFmepg channelLayout: \(self) order: \(order)")
        ...
        if let tag {
            ...
            MarblePlayerLog("can not find AudioChannelLayoutTag FFmepg channelLayout: \(self) order: \(order)")
...
extension NSError {
    convenience init(errorCode: MarblePlayerErrorCode, avErrorCode: Int32) {
